            ...
            steps {
                script {
                    withCredentials([
                        ...
                        string(credentialsId: 'PORT', variable: 'MONGO_PORT')
                    ]) {
                        // 👇 From here, all following stages can use these variables
                        runPipelineStages()
                    }
                }
            }
        ...
        steps {
            script {
                timeout(time: 10, unit: 'MINUTES') {
                    def qg = waitForQualityGate abortPipeline: true, credentialsId: "${SONAR_TOKEN}"
